...
public class ItemProfileEditServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!isActiveSession) {
			...
			ItemDto itemDto = itemServise.getItemDto(Long.parseLong(request.getParameter("idItem")));
			request.setAttribute("itemDto", itemDto);
			getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.ITEM_PROFILE_JSP.toString())
					.forward(request, response);
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!isActiveSession) {
			...
			ItemDto itemDto = new ItemDto(Long.parseLong(request.getParameter("idItem")),
					request.getParameter("nameItem"), request.getParameter("descriptionItem"));
			itemServise.setItemDto(itemDto,
					userService.getIdUserByLogin((LoginDto) (session.getAttribute("loginDto"))));
			getServletConfig().getServletContext().getRequestDispatcher(ControllerUrls.USER_ITEMS_SERVLET.toString())
					.forward(request, response);
		}
	}
