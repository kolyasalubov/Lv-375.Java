...
@WebServlet({"/favouriteMoviesList"})
public class FavouriteMoviesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session == null || session.getAttribute("login") == null || session.getAttribute("password") == null) {
			...
			int moviesCount = usersMovieService.getUsersMovieCount(user.getId());	        
	        int nOfPages = moviesCount / moviesOnPage;
	        if (moviesCount % moviesOnPage != 0) {
	        	nOfPages++;
	        }
	        
			List<MovieDto> movies = usersMovieService.getAllFavoriteMovies(user.getId(), moviesOnPage * currentPage - moviesOnPage, moviesOnPage);
			MoviesListDto moviesListDto = new MoviesListDto(movies, currentPage, nOfPages, moviesOnPage);
			//for navbar
