...
public class UserProfileServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session == null || session.getAttribute("login") == null || session.getAttribute("password") == null) {
			...
			if (user.getRole().equals(Role.ROLE_ADMIN.toString())) {dropdownValues.add(new Dropdown("Log Out", ControllerUrls.LOGOUT_SERVLET.toString()));
			dropdownValues.add(new Dropdown("Watch later", ControllerUrls.ADD_MOVIE_SERVLET.toString()));
			...
			getServletConfig()
			.getServletContext()
			.getRequestDispatcher(ViewUrls.USER_PROFILE_JSP.toString()).forward(request, response);

			//			response.sendRedirect(request.getContextPath()
//					+ ControllerUrls.USER_PROFILE.toString() + "?userId=" + userId);
		}		
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		System.out.println("****************new password" + newPassword + " new password is empty?: " + newPassword.isEmpty() );
		UsersDto userDto = new UsersDto(request.getParameter("fullName"), 
				 request.getParameter("role"),
				 newPassword.isEmpty() ? session.getAttribute("password").toString() : newPassword,
				 session.getAttribute("login").toString(),
				 Integer.valueOf(request.getParameter("isActive"))
							);
		...
		if(isUpdated) {
			...
			request.setAttribute("error", "Sorry, cannot update with such data");
			getServletConfig()
			.getServletContext()
